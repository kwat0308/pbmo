...
class PBMO:
    ...
    def __init__(self, dims=(10, 10), max_iter=10000, exclude_matrices=[]):
        ...
        self.normtimes = np.zeros(self.ntypes) if isinstance(self.dims, tuple) else np.zeros(
            (self.ntypes, len(self.dims)))
        self.matmultimes = np.zeros(self.ntypes) if isinstance(self.dims, tuple) else np.zeros(
            (self.ntypes, len(self.dims)))
    ...
    def evaluate_matmul(self, show_progress=False):
        ...
        # separate between single dimension case vs list of dimensions case
        if isinstance(self.dims, tuple):
            matmultimes = self.evaluate_matmul_dim(
                self.dims)
        else:
            ...
            for k, dim in enumerate(self.dims):
                if show_progress:
                    print("Current Matrix Dimension: {0} x {1}".format(*dim))
                matmul_time = self.evaluate_matmul_dim(
                    dim)
    ...
    def evaluate_matmul_dim(self, dim):
        ...
        for i in range(self.max_iter+1):
            ...
            rand_arr_1 = np.array(np.random.rand(
                *dim), copy=False).astype(np.float32)

            matrix_list_1 = self.initialize_matrices(
                rand_arr_1)

            for j, mat in enumerate(matrix_list):
                t0 = time.perf_counter_ns()
                norm_val = mat.matmul(matrix_list_1[j])
                t1 = time.perf_counter_ns()
    ...
    def plot_results(self, op_type="Norm", scale="linear", plot_ratios=False):
        ...
        if isinstance(self.dims, tuple):
            ...
            bar_text = ["{:.2e}".format(time_arr[j])
                        for j in range(self.ntypes)]

            fig = go.Figure([go.Bar(x=self.matrix_types, y=time_arr, text=bar_text,
                                    textposition='auto')])
            fig.update_layout(
                title={
                    "text": title,
            ...
            if plot_ratios:
                # format value seen on bar graph to scientific notation
                bar_text = ["{:.3e}".format(ratio_arr[j])
                            for j in range(self.ntypes)]

                fig_ratio = go.Figure([go.Bar(x=self.matrix_types, y=ratio_arr, text=bar_text,
                                              textposition='auto')])
                ...
                fig_ratio.write_html((os.path.join(
                    PLOT_PATH, "pbmo_{0}_barplot_ratios_{1}x{2}.png".format(op_type, *self.dims))))
            ...
            for j, mat_type in enumerate(self.matrix_types):
                fig.add_trace(go.Scatter(
                    x=dim_arr, y=time_arr[j], name=mat_type))
            ...
            if plot_ratios:
                ...
                for j, mat_type in enumerate(self.matrix_types):
                    fig_ratio.add_trace(go.Scatter(
                        x=dim_arr, y=ratio_arr[j], name=mat_type))
